...
if ( !class_exists( 'CurrentDateTimeWidget' ) ) {
	class CurrentDateTimeWidget {
		...
		function render() {
			...
			$currentDate = date( 'l j F' );
			$currentTime = date('g:i:s');
			
			
			echo '<div id="left_sidebar" class="left_sidebar">';
			
			if ( $title )
				echo '<h3 class="header_img">' . $title . '</h3>';
			
			echo '<p style="margin-bottom: 0px;" id="current-date-time">' . $currentDate . '</p>';
			echo '<h1 style="font-size: 3rem;">' . $currentTime . '</h1>';
			echo '</div>';
		}
