...
class W4_Post_list
{
	...
	function navigation( $max_num_pages, $paged = 1, $base = '', $attr = array() )
	{
		...
		{
			if( $paged == 1 )
			$return .= '<a href="'. add_query_arg( $pageq_var, ($max_num_pages) ) .'" class="prev page-numbers my_nepr"><</a>';
			else if( $paged == 2 )
				$return .= '<a href="'. remove_query_arg( array($pageq_var) ) .'" class="prev page-numbers my_nepr"><</a>';
			elseif( $paged > 2 )
				$return .= '<a href="'. add_query_arg( $pageq_var, ($paged - 1) ) .'" class="prev page-numbers my_nepr"><</a>';
			if( $max_num_pages > $paged )
				$return .= '<a href="'. add_query_arg( $pageq_var, ($paged + 1) ) .'" class="next page-numbers my_nepr">></a>';
			else
				$return .= '<a href="'. add_query_arg( $pageq_var, ($max_num_pages - ($paged-1)) ) .'" class="next page-numbers my_nepr">></a>';
$return .= '<a href="#" style="float: right;">More News</a>';
		}
